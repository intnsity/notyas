...
Sources: the six teardowns above, cross-read against `docs/plan-0.2.0/` in this repository (`UX.md`, `UX-SCREENS.md`, `PIN-MODES.md`, `VERIFY.md`). Where notyas already specifies a pattern I say so, because the useful output is the delta, not a restatement.
...
3. **Non-local press feedback: adopt. The randomized keypad itself: surveyed, built, and then deliberately declined.** Take the feedback rule first, because it is the half that survives. notyas C10 draws the press feedback on the dot row rather than on the key - the only control in the product with no local press feedback - and that is the transferable principle: feedback is deliberately non-local. Trezor's blind matrix (host shows blank positions, device shows the shuffled digit map) remains the strongest version of the shuffle idea, and per-attempt is the right granularity for it because per-keystroke shuffling causes mistaps. notyas shipped exactly that, put it on hardware, and **reversed it on 2026-08-19** (OPEN-QUESTIONS Q35): the pad is now fixed phone order, 1-2-3 / 4-5-6 / 7-8-9 over a 0. The project owner judged re-reading ten keys on every unlock too high a daily cost and accepted, explicitly and in writing, that fixed positions mean one clear look at his hand yields the PIN. Read this row as a divergence, not as a pattern: the vendors above are right that a shuffle is the only defence a device has against a passive observer, and notyas has chosen to have none. The feedback rule earns its keep on a different argument after the reversal - on a fixed pad a lit key is the digit itself, legible at a distance a fingertip is not.
