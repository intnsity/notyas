...
//! Board: Waveshare ESP32-P4-WIFI6-Touch-LCD-10.1 (800x1280 10.1inch
//! MIPI-DSI, JD9365; "X" series shared PCB). PORTRAIT panel - see the boot
//! warning in radio_lockdown.
//!
...
//!   (C6-MINI-1U-H8 like the 4B), no EN pullup; 10.1 inch panel = JD9365,
...
//!   @ be0e5e4, re-fetched 2026-08-17),
//!   CONFIG_BSP_LCD_TYPE_800_1280_10_1_INCH branch: LCD RESET GPIO27,
//!   backlight LEDC PWM GPIO26 (non-inverted), touch RST/INT = NC; DPI
//!   timing = JD9365_800_1280_PANEL_60HZ_DPI_CONFIG (80 MHz, HBP 20 /
//!   HPW 20 / HFP 40, VBP 10 / VPW 4 / VFP 30, use_dma2d - the same values
//!   as waveshare/esp_lcd_jd9365_10_1's own macro);
//!   esp_lcd_panel_disp_on_off(true) after init.
//! - Panel driver + init commands: the BSP feeds a 10.1-inch-specific
//!   201-entry table to the generic esp_lcd_jd9365 driver. This module uses
//!   waveshare/esp_lcd_jd9365_10_1 v2.0.0 (Waveshare's driver for this
//!   exact panel; exports esp_lcd_new_panel_jd9365 / jd9365_vendor_config_t
//!   without a _10_1 suffix) with init_cmds NULL: its built-in default
//!   table was verified (2026-08-17) entry-for-entry equal to the BSP's
//!   10.1-inch table apart from a duplicated leading page-select no-op
//!   ({0xE0, 0x00} sent twice instead of once).
...
pub const BOARD_NAME: &str = "Waveshare ESP32-P4-WIFI6-Touch-LCD-10.1 (UNTESTED)";
...
// X BSP display.h (10.1 inch branch): 1500 Mbps/lane - the top of the P4's
...
fn try_display_init() -> Result<Display, DisplayError> {
    ...
    // 10.1-inch table (verified equal; see module banner).
    let mut vendor_config = sys::jd9365_vendor_config_t::default();
    ...
    esp_check!(
        sys::esp_lcd_new_panel_jd9365(io, &panel_config, &mut panel),
